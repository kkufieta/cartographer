...
#include "cartographer/io/read_from_file.h"
...
class MapBuilderViam {
  ...
  cartographer::sensor::TimedPointCloudData GenerateSavedRangeMeasurements(double travel_distance, double duration, double time_step, std::string initial_filename, int i);
  cartographer::sensor::TimedPointCloudData GenerateSaved2DRangeMeasurements(const Eigen::Vector3f& translation, double duration, double time_step, const transform::Rigid3f& local_to_global, std::string initial_filename, int i);
  
  cartographer::sensor::TimedPointCloudData GetDataFromFile(std::string initial_filename, int i);
